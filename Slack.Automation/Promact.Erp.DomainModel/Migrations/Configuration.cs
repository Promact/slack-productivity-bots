    ...
    public class Configuration : DbMigrationsConfiguration<Promact.Erp.DomainModel.Context.PromactErpContext>
    {
        ...
        protected override void Seed(Promact.Erp.DomainModel.Context.PromactErpContext context)
        {
            if (context.Question.Count() == 0)
            {
                ...
                Question sixthQuestionTaskMail = new Question() { CreatedOn = DateTime.UtcNow, OrderNumber = 6, QuestionStatement = "Are you sure to send mail? After sending email you won't be able to add any task for today.", Type = 2 };
                ...
                Question secondQuestionScrumBot = new Question() { CreatedOn = DateTime.UtcNow, OrderNumber = 2, QuestionStatement = "How you are going to rock it today?", Type = 1 };
