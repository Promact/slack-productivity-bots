...
namespace Promact.Erp.Web
{
    public static class BotStartUp
    {
        public static async Task StartUpAsync(IComponentContext container)
        {
            ...
            taskMailBotRepository.StartAndConnectTaskMailBot((await appCredential.FetchAppCredentialByModuleAsync(stringConstant.TaskModule)).BotToken);
            scrumBotRepository.StartAndConnectScrumBot((await appCredential.FetchAppCredentialByModuleAsync(stringConstant.Scrum)).BotToken);
        }
    }
}