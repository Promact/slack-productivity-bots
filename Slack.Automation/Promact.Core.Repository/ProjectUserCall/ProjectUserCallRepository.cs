...
namespace Promact.Core.Repository.ProjectUserCall
{
    public class ProjectUserCallRepository : IProjectUserCallRepository
    {
        ...
        public async Task<List<UserRoleAc>> GetUserRole(string slackUserId, string accessToken)
        {
            var requestUrl = string.Format("{0}{1}", _stringConstant.ProjectInformationUrl, slackUserId);
            var response = await _httpClientRepository.GetAsync(_stringConstant.ProjectUrl, requestUrl, accessToken);
            var Json = JsonConvert.DeserializeObject<List<UserRoleAc>>(response);
            return Json;
        }

        public async Task<List<UserRoleAc>> GetListOfEmployee(string slackUserId, string accessToken)
        {
            var requestUrl = string.Format("{0}{1}", _stringConstant.ProjectUasrInformationUrl, slackUserId);
            var response = await _httpClientRepository.GetAsync(_stringConstant.ProjectUrl, requestUrl, accessToken);
